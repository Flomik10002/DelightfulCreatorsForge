...
/**
 * Renders our own copy of the Mechanical Press body/pole model with a spinning blade head (see
 * ModPartialModels + assets/delightfulcreators/models/block/mechanical_cutter/*.json), kept in our
 * own namespace rather than referenced directly from Create so it's ours to edit. The blade uses
 * vanilla's animated stonecutter_saw texture, so it spins on its own via Minecraft's texture
 * animation - no per-frame rotation code needed here. The head is rendered into
 * {@link RenderType#cutout()} rather than solid() precisely because that texture has hard alpha
 * edges - the model's own "render_type" JSON field is NOT read here, since this SuperByteBuffer is
 * drawn into a manually chosen buffer instead of going through the normal model render-layer
 * pipeline.
 *
